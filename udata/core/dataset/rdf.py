...
def format_from_rdf(resource: RdfResource):
    ...
    - For other types, return the distribution format.
    """
    ...
    if service := resource.value(DCAT.accessService):
        # Support both GEODCAT.serviceProtocol and DCT.conformsTo mappings.
        # Using chain() instead of objects(...|...) to enforce deterministic order.
        for standard in chain(
            service.objects(GEODCAT.serviceProtocol), service.objects(DCT.conformsTo)
        ):
            # FIXME: do we want to cover invalid cases where Standard is not a URIRef?
...
def title_from_rdf(resource: RdfResource, url: str | None = None, format: str | None = None) -> str:
    ...
    As it's not a mandatory property, it falls back on building a title from the URL, then the
    format, and in last ressort a generic resource name.
    """
